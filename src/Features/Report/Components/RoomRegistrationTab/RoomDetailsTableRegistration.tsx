...
const RoomDetailsTableRegistration = ({
  roomDetails,
  onDelete,
}: {
  roomDetails: IRoomDetailsRegistration[];
  onDelete?: (index: number) => void;
}) => {
  const roomDetailsColumns: ColumnDef<IRoomDetailsRegistration>[] = [
    {
      accessorKey: "index",
      header: "S/N",
    ...
    {
      accessorKey: "roomDepartureDate",
      header: "Checkout Date",
      cell: ({ row }) => (
        <span>
          {row.original?.roomDepartureDate
            ? new Date(row.original.roomDepartureDate).toLocaleDateString()
            : "-"}
        </span>
      ),
    },
...
export default RoomDetailsTableRegistration;
